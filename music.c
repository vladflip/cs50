...
int frequency(string note)
{
    ...
    int frequency = (int)round(440 * pow(2, octave - 4));
    int semitones = count_semitones(note[0]);
    float semitone = pow(2, (float)1 / (float)12);

    if (letter != 'A')
    {
        frequency = (int)round((float)frequency * pow((float)2, (float)semitones/(float)12));
    }

    if (sharp)
    {
        frequency = (int)round((float)frequency * semitone);
    }
    else if(flat)
    {
        frequency = (int)round((float)frequency / semitone);
